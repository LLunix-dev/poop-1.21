...
import com.llunix.poop.block.custom.TinyPileBlock;
...
public class ModBlocks {
    ...
    private static Block registerBlock(String name, Block block) {
        ...
        return Registry.register(Registries.BLOCK, Identifier.of(Poop.MOD_ID, name), block);
    }

    private static void registerBlockItem(String name, Block block) {
        Registry.register(Registries.ITEM, Identifier.of(Poop.MOD_ID, name),
                new BlockItem(block, new Item.Settings()));
    }

    public static void registerModBlocks() {
        Poop.LOGGER.info("Registering Mod Blocks for " + Poop.MOD_ID);

        ItemGroupEvents.modifyEntriesEvent(ItemGroups.BUILDING_BLOCKS).register(entries -> {
            entries.add(POOP_BLOCK);
            entries.add(PILE);
            entries.add(TINY_PILE);
        });
    }
}
